...
| Option                  | `a11y.role:String\|false`                          | `'popup'`        | Inject `role` and related attributes, can be `'popup'`, `'dialog'`, `'tooltip'`, `'carousel'`, also if has container and targets and no divs between `'tablist'`, `'menu'`, `'listbox'`            |
...
| Option                  | `a11y.hidden:Boolean`                          | `false`        | Inject `aria-hidden` attributes on targets depending on activation            |
| Option                  | `a11y.live:Boolean`                          | `false`        | Inject `aria-live` attributes on auto            |
...
Use `on:z-above` to have the button above the backdrop.
