...
To have `.container` fluid so with only one `max-width` in Tailwind CSS preset set `--container-fluid: 1536px;` with the desired `max-width`, see [Tailwind CSS container](https://tailwindcss.com/docs/container).
