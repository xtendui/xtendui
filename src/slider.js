...
class Slider extends Xt.Toggle {
  ...
  initGroups() {
          ...
          if (left >= min) {
            ...
            if (i > firstIndex) {
              ...
              for (const target of group.targets) {
                self.group[firstIndex].targets.push(target) // put at end
        ...
        for (const slide of group.targets) {
          ...
          if (left <= max) {
            ...
            if (i < lastIndex) {
              ...
              for (const target of group.targets) {
                self.group[lastIndex].targets.unshift(target) // put at start
                target.setAttribute('data-xt-group', groupStr)
              }
            }
    ...
    for (const group of self.group) {
      const slide = group.target
      Xt.dataStorage.set(slide, `${self.ns}GroupInitial`, slide.getAttribute('data-xt-group'))
    }
    ...
    if (options.mode !== 'absolute') {
      for (const [z, group] of self.group.entries()) {
        const slide = group.target
        const groupGroup = Xt.dataStorage.get(slide, `${self.ns}GroupInitial`)
  ...
  initPagination() {
    ...
    for (const [z, pag] of pags.entries()) {
      ...
      for (const [i, group] of self.group.entries()) {
        ...
        item.children[0].setAttribute('data-xt-group', Xt.dataStorage.get(group.target, `${self.ns}GroupInitial`))
        ...
        self.group[i].element = self.pags[0][i]
      }
    }
  }
  ...
  eventOn(element, force = false, e = null) {
    ...
    if (options.mode === 'absolute' && options.loop) {
      if (/*self.detail.dragDirection < 0 &&*/ self.detail.dragFinalOld > min) {
        self.initial = true
        self.detail.dragFinal = max - min + self.detail.dragFinalOld - Xt.dataStorage.get(first, `${self.ns}GroupWidth`)
        // listener dispatch
        self.dragger.dispatchEvent(new CustomEvent(`dragposition.${self.componentNs}`))
        self.initial = false
      } else if (/*self.detail.dragDirection > 0 &&*/ self.detail.dragFinalOld < max) {
        self.initial = true
        self.detail.dragFinal = min - max + self.detail.dragFinalOld + Xt.dataStorage.get(first, `${self.ns}GroupWidth`)
        //console.log(self.detail.dragFinal, min, max, self.detail.dragFinalOld, Xt.dataStorage.get(first, `${self.ns}GroupWidth`))
        // listener dispatch
        self.dragger.dispatchEvent(new CustomEvent(`dragposition.${self.componentNs}`))
        self.initial = false
      }
