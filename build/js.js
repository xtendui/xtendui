...
const jsThemesGlob = new glob.Glob('src/themes/**/*.js', function(er, files) {
  for (const file of files) {
...
jsThemesGlob.on('end', function(filepath) {
  writeFile('./src/xtend-themes.js', jsThemes, function(err) {
    if (err) console.log(err)
  })
})
