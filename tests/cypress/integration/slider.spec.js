  ...
  it('TEST grouping and activation, wrap position, performance slider.', function () {
    cy.get(container)
      ...
      .then(() => {
        ...
        expect(self.targets[0].firstElementChild.style.display).to.equal('')
        expect(self.targets[1].firstElementChild.style.display).to.equal('')
        expect(self.targets[2].firstElementChild.style.display).to.equal('')
        expect(self.targets[3].firstElementChild.style.display).to.equal('none')
        expect(self.targets[4].firstElementChild.style.display).to.equal('none')
        expect(self.targets[5].firstElementChild.style.display).to.equal('')
      })
      .get(self.elements[4])
      .click()
      .wait(750) // racecondition special
      .then(() => {
        ...
        expect(self.targets[0].firstElementChild.style.display).to.equal('none')
        expect(self.targets[1].firstElementChild.style.display).to.equal('none')
        expect(self.targets[2].firstElementChild.style.display).to.equal('none')
        expect(self.targets[3].firstElementChild.style.display).to.equal('')
        expect(self.targets[4].firstElementChild.style.display).to.equal('')
        expect(self.targets[5].firstElementChild.style.display).to.equal('')
        expect(self.targets[6].firstElementChild.style.display).to.equal('')
      })
  })
})
